...
LRESULT CALLBACK SystemClass::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_KEYUP:
	{
		// If a key is released then send it to the input object so it can unset the state
		// for that key.
		m_Input->KeyUp((unsigned int)wparam);
		return 0;
	}
	// Any other messages send to the default message handler as our application won't make
	// use of them.
	default:
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	// Setup the screen settings depending on whether it is running in
